...
bbGuild WoW is the World of Warcraft plugin for bbGuild, a guild
management extension for phpBB with roots in bbDKP and EQdkp. It brings
WoW guild management into your forum, connecting to Blizzard's
Battle.net API to synchronize guild rosters, character profiles,
specializations, equipment, and portraits. With support for Retail and
Classic editions, plus an integrated achievement browser and guild
activity feed, it helps your community keep track of its characters and
...
See the [Installation Guide](INSTALL.md) for detailed setup instructions including Battle.net API configuration.
...
The API client uses OAuth 2.0 Client Credentials Grant with the modern `api.blizzard.com` endpoints and supports all five regions (US, EU, KR, TW, SEA). See the [Battle.net API Reference](BATTLENET_API.md) for details.

## Where to start

- New here? Start with **[Installation](INSTALL.md)**.
- Hitting an issue? Check the **[FAQ](FAQ.md)**.
- Building a plugin for another game? This extension is the reference
  implementation — see **[Architecture](ARCHITECTURE.md)** for the
  plugin contract and structure.
- Want the older Community API shape for reference? See
  [Blizzard API — Retail](blizzard-api-retail.md) and
  [Blizzard API — Classic](blizzard-api-classic.md).
- Curious what's changed? Check the **[Changelog](CHANGELOG.md)**.

## Contributing

See [`CONTRIBUTING.md`](https://github.com/avatharbe/bbguildwow/blob/main/CONTRIBUTING.md)
for the full contribution policy, and
[`SECURITY.md`](https://github.com/avatharbe/bbguildwow/blob/main/SECURITY.md)
